...
AVISTATUS avi_init(uint8_t *buf, uint32_t size)
{
    ...
    ESP_LOGI(avi, "g_avix.SecPerFrame:%ld\r\n", g_avix.SecPerFrame);
    ESP_LOGI(avi, "g_avix.TotalFrame:%ld\r\n", g_avix.TotalFrame);
    ESP_LOGI(avi, "g_avix.Width:%ld\r\n", g_avix.Width);
    ESP_LOGI(avi, "g_avix.Height:%ld\r\n", g_avix.Height);
    ESP_LOGI(avi, "g_avix.AudioType:%d\r\n", g_avix.AudioType);
    ESP_LOGI(avi, "g_avix.SampleRate:%ld\r\n", g_avix.SampleRate);
    ESP_LOGI(avi, "g_avix.Channels:%d\r\n", g_avix.Channels);
    ESP_LOGI(avi, "g_avix.AudioBufSize:%d\r\n", g_avix.AudioBufSize);
    ESP_LOGI(avi, "g_avix.VideoFLAG:%s\r\n", g_avix.VideoFLAG);
    ESP_LOGI(avi, "g_avix.AudioFLAG:%s\r\n", g_avix.AudioFLAG);
...
AVISTATUS avi_get_streaminfo(uint8_t *buf)
{
    ...
    {
        ESP_LOGI(avi, "FRAME SIZE OVER:%ld\r\n", g_avix.StreamSize);
        g_avix.StreamSize = 0;
