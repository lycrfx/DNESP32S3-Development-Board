...
//    text_show_string(10, 10, spilcddev.width - 10, 16, (char *)buf, 16, 0, RED);   /* 显示文件名 */
    
    sprintf((char *)buf, "索引:%d/%d", index, total);
//    text_show_string(10, 30, spilcddev.width - 10, 16, (char *)buf, 16, 0, RED);   /* 显示索引 */
...
static void video_info_show(AVI_INFO *aviinfo)
{
    ...
    text_show_string(10, 10, spilcddev.width - 10, 16, (char *)buf, 16, 0, RED);   /* 显示歌曲名字 */
    
    sprintf((char *)buf, "帧率:%ld帧", 1000 / (aviinfo->SecPerFrame / 1000));
    text_show_string(10, 30, spilcddev.width - 10, 16, (char *)buf, 16, 0, RED);   /* 显示歌曲名字 */
...
void video_time_show(FIL *favi, AVI_INFO *aviinfo)
{
    ...
    if (oldsec != cursec)                                           /* 需要更新显示时间 */
    {
        ...
        text_show_string(10, 50, spilcddev.width - 10, 16, (char *)buf, 16, 0, RED);   /* 显示歌曲名字 */
...
static uint8_t video_play_mjpeg(uint8_t *pname)
{
    ...
    while (res == 0)
    {
        ...
        if (res == 0)
        {
            ...
            res = mjpegdec_init((spilcddev.width - g_avix.Width) / 2, 110 + (spilcddev.height - 110 - g_avix.Height) / 2);/* 初始化JPG解码 */
            /* 定义图像的宽高 */
...
void video_play(void)
{
    ...
    while (f_opendir(&vdir, "0:/VIDEO") != FR_OK)                               /* 检查VIDEO文件夹是否存在 */
    {
        ...
        vTaskDelay(200);
        spilcd_fill(60, 190, 240, 206, WHITE);
        vTaskDelay(200);
    }
    ...
    while (totavinum == 0)
    {
        ...
        vTaskDelay(200);
        spilcd_fill(60, 190, 240, 146, WHITE);
        vTaskDelay(200);
    }
